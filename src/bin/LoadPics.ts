...
export default async function loadPics(client: ExtendedClient): Promise<void> {
  ...
  client.picIds = picEmbeds.map((file) => (file.split("/").pop() as string).split(".")[0]);
  client.picIdsTrash = picEmbedsTrash.map((file) => (file.split("/").pop() as string).split(".")[0]);
  ...
  picFileNames.map((line) => {
    ...
    if (picId) client.picFileNames.set(picId, fileName);
  });
